...
class RevisionDAO {
    ...
    function crearRevision(Revision $obj){
        ...
        if(empty($version) || $version[0]->{"version"} == null){
            $version = 1;
        } else {
        ...
        $query = "INSERT INTO tb_revisiones (revisor_id,articulo_id,estado_id,version,url,descripcion,fecha) VALUES ('"
            .$obj->getRevisor()."', ".$obj->getArticulo().", ".$estado[0]->{"id"}.",".$version.",'".$url."','"
            .$obj->getDescripcion()."','".$fecha."');";
        $resp = json_decode($this->repository->ExecuteTransactionAux($query));
        echo(json_encode(['status' => 'true', "msg" => $url]));
    }

    function responder($revision, $mensaje){
        $query = "UPDATE tb_revisiones set mensaje='".$mensaje."' WHERE id=".$revision.";";
        $this->repository->ExecuteTransaction($query);    
    }

    function buscar($idRevision){
        $query = "SELECT r.id,r.fecha,r.descripcion,r.url,r.version,r.mensaje,r.estado_id,u.nombre,u.apellido,e.descripcion as estado,r.revisor_id as revisor FROM tb_revisiones r JOIN tb_usuarios u ON u.id=r.revisor_id JOIN tb_estados_revision e ON e.id=r.estado_id WHERE r.id=".$idRevision.";";
